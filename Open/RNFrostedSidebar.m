    ...
    if (self = [super init]) {
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
			...
			if (labels) {
				...
				label.textColor = kSystemMenuLabelColor;
				label.font = kSystemMenuFont;
...
//    CGFloat parentWidth = self.view.bounds.size.width;
    CGFloat parentWidth = self.view.bounds.size.height;
    
    CGRect contentFrame = self.view.bounds;
//    contentFrame.origin.x = _showFromRight ? parentWidth : -_width;

    contentFrame.origin.y = _showFromRight ? parentWidth : - _height;
...
//    CGRect blurFrame = CGRectMake(_showFromRight ? self.view.bounds.size.width : 0, 0, 0, self.view.bounds.size.height);
    CGRect blurFrame = CGRectMake(_showFromRight ? self.view.bounds.size.width : 0, 0, self.view.frame.size.width, 0);
...
//    contentFrame.origin.x = _showFromRight ? parentWidth - _width : 0;
    ...
    blurFrame.origin.y = contentFrame.origin.x;
...
//        blurFrame.origin.x = self.showFromRight ? parentWidth : 0;
        ...
        blurFrame.origin.y = self.showFromRight ? parentWidth : 0;
...
- (void)layoutSubviews {
    ...
    self.contentView.frame = CGRectMake(x, 0, _width, _height);
...
- (void)layoutItems {
	...
	[self.labels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL *stop) {
        CGRect frame = CGRectMake(40, topPadding*idx + self.itemSize.height*(idx) + topPadding + 14, self.width, 24);
        label.frame = frame;
    }];
