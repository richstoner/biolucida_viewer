...
file_list = reversed(glob.glob('/Users/stonerri/octopress/source/%s/*.plist' % deploy_name))
...
for f in file_list:
    ...
    if basename not in web_dict.keys():
        ...
        a = {}
        a['url'] = 'itms-services://?action=download-manifest&url=https://richstoner.github.io/%s/%s' % (deploy_name, f.split('/')[-1])
        a['target'] = 'Adhoc'
        a['version'] = f.split('/')[-1].split('-')[2].split('.')[0]
        a['build'] = f.split('/')[-1].split('-')[3].split('.')[0]
        a['dllink'] = 'https://richstoner.github.io/%s/%s' % (deploy_name, f.split('/')[-1].replace('.plist', '.ipa'))
        ...
        a = {}
        a['url'] = 'itms-services://?action=download-manifest&url=hhttps://richstoner.github.io/%s/%s' % (deploy_name, f.split('/')[-1])
        a['target'] = 'Adhoc'
        a['version'] = f.split('/')[-1].split('-')[2].split('.')[0]
        a['build'] = f.split('/')[-1].split('-')[3].split('.')[0]
        a['dllink'] = 'https://richstoner.github.io/%s/%s' % (deploy_name, f.split('/')[-1].replace('.plist', '.ipa'))
...
output_file = '/Users/stonerri/octopress/source/%s/index.html' % (deploy_name)
